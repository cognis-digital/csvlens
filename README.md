...
[![PyPI](https://img.shields.io/pypi/v/cognis-csvlens.svg)](https://pypi.org/project/cognis-csvlens/)
[![CI](https://github.com/cognis-digital/csvlens/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/csvlens/actions)
...
CSVLENS exists for one job — fast cli for profiling and cleaning huge csv / parquet files — and does it without a SaaS bill or heavyweight setup.
...
```bash
csvlens --version
csvlens scan .                      # scan the current project
csvlens scan . --format json
csvlens scan . --fail-on high       # non-zero exit for CI gates
csvlens mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **xsv / qsv**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`csvlens` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
