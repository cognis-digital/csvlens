"""Native cognis-connect emit for csvlens — forward findings to any platform.

Maps csvlens's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    csvlens ... --format json | csvlens-emit --to stix
    csvlens-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('column', '') + ' - ' + rec.get('value', '')
        out['severity'] = 'low'
        out['type'] = 'data'
        out['description'] = f"Value {rec.get('value', '')} found in column {rec.get('column', '')}"
        out['tags'] = [rec.get('column', '')]
